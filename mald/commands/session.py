...
def _start_tmux_session(session_name, tmux_config, env):
    ...
    subprocess.run(
        ["tmux", "kill-session", "-t", session_name], capture_output=True, env=env
    )
    ...
    cmd = [
        ...
        session_name,
        "-c",
        env.get("MALD_HOME", str(Path.home() / ".mald")),
    ]
...
def _setup_tmux_windows(session_name, env):
    ...
    subprocess.run(
        ["tmux", "new-window", "-t", session_name, "-n", "editor", "nvim"], env=env
    )

    # Window 2: Terminal
    subprocess.run(
        ["tmux", "new-window", "-t", session_name, "-n", "terminal"], env=env
    )
