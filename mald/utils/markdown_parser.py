    ...
    def get_backlinks(self, kb_path: Path):
        ...
        for md_file in kb_path.glob("**/*.md"):
            ...
            try:
                ...
                # Match both exact name and variant with spaces/underscores
                target_variants = [re.escape(target_name)]
                alt_name = target_name.replace("_", " ")
                if alt_name != target_name:
                    target_variants.append(re.escape(alt_name))
                wikilink_pattern = (
                    rf'\[\[(?:{"|".join(target_variants)})(\|[^\]]+)?\]\]'
                )
                if re.search(wikilink_pattern, content, re.IGNORECASE):
...
def find_orphaned_files(kb_path: Path, documents: Dict = None):
    ...
    for doc in documents.values():
        for link in doc.links:
            ...
            # Also add normalized version (lowercase, spaces to underscores)
    ...
    for rel_path, doc in documents.items():
        ...
        if (
            rel_path not in linked_files
            and rel_normalized not in linked_files
            and rel_path != "index.md"
        ):
            orphaned.append(doc)
...
def generate_graph_data(kb_path: Path):
    ...
    for rel_path, doc in documents.items():
        ...
        for link in doc.links:
            ...
            # Also try normalized version (lowercase, spaces to underscores)
            normalized = target_path.lower().replace(" ", "_")
            matched = (
                target_path
                if target_path in documents
                else (normalized if normalized in documents else None)
            )
            if matched is not None:
                edges.append({"source": rel_path, "target": matched, "type": "link"})
...
def search_content(
    ...
    for rel_path, doc in documents.items():
        ...
        for match in re.finditer(re.escape(query), doc.content, flags):
            ...
            matches.append({"position": match.start(), "context": context.strip()})

        if matches:
            results.append(
                {"document": doc, "matches": matches, "match_count": len(matches)}
            )
