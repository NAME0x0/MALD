...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--version", action="version", version=f"mald {__version__}")
    ...
    session_parser = subparsers.add_parser(
        "session", help="Start MALD interactive session"
    )
    ...
    iso_wsl = iso_subparsers.add_parser("wsl", help="Build WSL distro tarball")
    iso_wsl.add_argument("--output", "-o", help="Output directory")
...
def main() -> int:
    ...
    try:
        ...
        if args.command == "init":
            return init.handle(args) or 0
        elif args.command == "kb":
            return kb.handle(args) or 0
        elif args.command == "session":
            return session.handle(args) or 0
        elif args.command == "iso":
            return iso.handle(args) or 0
        elif args.command == "ai":
            return ai.handle(args) or 0
        elif args.command == "config":
            return config.handle(args) or 0
        else:
